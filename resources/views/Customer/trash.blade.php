...
<div class="row">
    <div class="card mb-4">
        ...
        <div>
            <center >
                <h1 class="fw-bold textcolor-danger" style="color: red">Trash Page</h1>
            </center>
        </div>
        <br>
        <div class="card-body">
...
@section('CustomerTrash')
<script>
    $(document).ready(function(){
     var table=  $('.customerTable').DataTable({
        processing:true,
        serverSide:true,
        ajax:"{{ route('customer.trashData') }}",
        columns:[
        {data:'name'},
        {data:'email'},
        {data:'address'},
    ...
    $('body').on('click','.restore',function(){
           ...
           swal({
                  ...
                  if (willRestore) {
                      ...
                      $.ajax ( {
                          type:"POST",
                          url:"/customer/trashRecovery/"+restore_id,
                          data: {
          ...
          $('body').on('click','.dltCustomer',function(){
           ...
           swal({
                  ...
                  if (willDelete) {
                      ...
                      $.ajax ( {
                          type:"DELETE",
                          url:"/customer/trashDelete/"+delete_id,
                          data: {
                              "_token":$('input[name="_token"]').val(),//passing token for deleting
                              "id":delete_id,
                          },
